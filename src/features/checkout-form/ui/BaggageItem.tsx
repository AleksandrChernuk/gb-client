...
export const BaggageItem = ({ icon, title, description, price, count, onIncrease, onDecrease, currency }: Props) => {
  ...
  return (
    <div className="flex items-center gap-2 justify-between bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl shadow-sm">
      ...
      {price !== undefined && (
        <div className="flex items-center gap-2">
          {(count || 0) * price !== 0 && count && (
            <div className="text-sm dark:text-slate-50  ">
              {count * price}
              <span className="text-green-300 ml-0.5 font-light">{currency}</span>
            </div>
          )}
