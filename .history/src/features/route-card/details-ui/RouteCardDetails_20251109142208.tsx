              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: routeDetails.routeInfo
                    // 1️⃣ Видаляємо усі HTML-посилання
                    .replace(/<a\b[^>]*>(.*?)<\/a>/gi, '$1')
                    // 2️⃣ Видаляємо сирі URL (http, https, www, mailto)
                    .replace(/\b(?:https?:\/\/|www\.|mailto:)[^\s<]+/gi, '')
                    // 3️⃣ Розбиваємо текст по маркеру ●
                    .split('●')
          ...
          <RouteDetailsSection label={t('amenities')} listClassName="flex-row flex-wrap gap-1">
            <RouteDetailsField>
              <div className="flex flex-row items-center flex-wrap gap-1">
                {translateAmenitiesList(routeDetails.amenities, providerName, t).map((item, i) => (
